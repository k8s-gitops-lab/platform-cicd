...
make argocd-apps-render     # Regénérer apps-appset.yaml depuis l'inventaire
make check-generated        # Vérifier que apps-appset.yaml est à jour
...
| `scripts/platform_inventory.py` | Modèle de données partagé avec `toolbox` |
| `scripts/render-argocd-apps.py` | Génère `platform-gitops/argocd/managed/apps-appset.yaml` |
...
- **`argocd/managed/` dans `platform-gitops` est généré** — ne jamais l'éditer
  à la main. Utiliser `make argocd-apps-render` puis committer.
...
- Ne pas éditer `argocd/managed/apps-appset.yaml` dans `platform-gitops`
  directement — passer par `make argocd-apps-render`.
